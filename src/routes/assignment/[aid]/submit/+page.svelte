		...
		<div class="form w-full mx-auto">
			<form id="form" method="POST" use:enhance={handleSubmit} bind:this={submissionForm}>
				...
				{#if rubric.require_self_assessment}
					<div class="pt-4">
						...
						{#each range(1, rubric.areas.length) as a, i}
							...
							>
								<legend class="font-semibold">{rubric.areas[i].name}</legend>
								<div class="flex flex-col md:flex-row">
									...
									>
										...
										<div class="flex items-center">
											<input class="radio rounded-full" type="radio" id="areas[{i}].level[{j}]" name="areas[{i}].level" bind:group={areas[i].level} value={j} />
											<label class="pl-2" for="areas[{i}].level[{j}]">{rubric.levels[j].name}</label>
										</div>
										{/each}
										</fieldset>
									</div>
									<div class="grow md:basis-2/3 px-4">
										<label for="areas[{i}].comment" class="font-semibold">Comments</label>
										<textarea id="areas[{i}].comment" class="textarea" rows={rubric.levels.length} bind:value={areas[i].comment}></textarea>
									</div>
								</div>
							</fieldset>
						{/each}
				...
				<div class="my-2 bg-surface-200 dark:bg-surface-700 rounded-lg">
					...
					<div class="flex items-center">
						<label class="px-2 py-2 text-xl" for="complete">Tick this box if your submission is complete.</label>
						<input class="px-2 py-2 checkbox" type="checkbox" form="form" id="complete" name="complete" bind:checked={complete} />
					</div>
				</div>
			</form>
		</div>
